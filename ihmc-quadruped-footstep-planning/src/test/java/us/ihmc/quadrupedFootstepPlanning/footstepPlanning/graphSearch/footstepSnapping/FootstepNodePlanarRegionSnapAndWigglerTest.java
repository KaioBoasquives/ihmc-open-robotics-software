...
//      testABunchOfPoints(dataSet, 5000);
   ...
   private static void testPoint(Random random, Point3DReadOnly containingPointOfSuspicion, DataSet dataSet, int iters)
   {
      ...
      DefaultFootstepPlannerParameters plannerParameters = new DefaultFootstepPlannerParameters()
      {
         @Override
      ...
      FootstepNodePlanarRegionSnapAndWiggler snapAndWiggler = new FootstepNodePlanarRegionSnapAndWiggler(plannerParameters, () -> distanceInside, () -> useConvexHull,
                                                                                                         careAboutGridCell);
...
//            normal.scale(5.0e-3);
            ...
            int xIndex = FootstepNode.snapToGrid(pointOnEdgeInWorld.getX());
            int yIndex = FootstepNode.snapToGrid(pointOnEdgeInWorld.getY());
            double gridX = FootstepNode.gridSizeXY * xIndex;
            double gridY = FootstepNode.gridSizeXY * yIndex;
            ...
            FootstepNodeSnapData snapData = snapAndWiggler.snapFootstepNode(RobotQuadrant.FRONT_LEFT, xIndex, yIndex, 0.0);
      ...
      for (int iter = 0; iter < iters; iter++)
      {
         ...
         FootstepNodeSnapData snapData = snapAndWiggler.snapFootstepNode(robotQuadrant, xIndex, yIndex, yaw);
