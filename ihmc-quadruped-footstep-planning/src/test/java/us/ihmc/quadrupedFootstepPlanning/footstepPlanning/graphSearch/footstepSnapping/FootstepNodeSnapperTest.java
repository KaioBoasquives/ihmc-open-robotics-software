...
public class FootstepNodeSnapperTest
{
                     ...
                     for (int n = 0; n < hindLeftYIndices.length; n++)
                     {
                        for (int o = 0; o < hindRightXIndices.length; o++)
                        {
                           for (int p = 0; p < hindRightYIndices.length; p++)
                           {
                              ...
                              testSnapper.snapFootstepNode(robotQuadrant, xIndex, yIndex, 0.0);
                     ...
                     for (int n = 0; n < hindLeftYIndices.length; n++)
                     {
                        for (int o = 0; o < hindRightXIndices.length; o++)
                        {
                           for (int p = 0; p < hindRightYIndices.length; p++)
                           {
                              ...
                              FootstepNodeSnapData snapData = testSnapper.snapFootstepNode(newQuadrant, xIndex, yIndex, 0.0);
   ...
   public void testHashMapStorage()
   {
      ...
      for (int i = 0; i < numTrials; i++)
      {
         ...
         nodeB = new FootstepNode(robotQuadrant, frontLeft, otherFrontRight, otherHindLeft, otherHindRight, yaw, 1.5, 0.5);
   ...
   private class TestSnapper extends FootstepNodeSnapper
   {
      ...
      @Override
      protected FootstepNodeSnapData snapInternal(RobotQuadrant robotQuadrant, int xIndex, int yIndex, double yaw)
      {
         dirtyBit = true;
